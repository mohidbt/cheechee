...
export function Console({ state, tracks, engine, connected, services, busy, activities, onCommand, onSubmit, onStopAll, onImport, voiceControls, speechToggle }: ConsoleProps) {
  ...
  return <main className="dj-app">
    <header className="app-header"><div className="brand"><span className="brand-mark" aria-hidden="true"><i/><i/><i/><i/></span><div><strong>AFTERHOURS</strong><span>AI DJ console</span></div></div><div className="header-actions"><span className={`connection-state ${agentReady ? 'online' : ''}`}><span className="status-dot" />{agentReady ? `Agent online${services?.model ? ` · ${services.model}` : ''}` : !connected ? 'Agent disconnected' : 'Agent unavailable'}</span><button className="stop-all" onClick={onStopAll}>■ <span>Stop all audio</span></button></div></header>
    ...
    <div className="console-shell"><div className="mixing-surface"><Deck id="A" deck={state.decks.A} track={currentTrack('A')} tracks={tracks} engine={engine} onCommand={onCommand} />
